...
void AShotgun::FireShotgun(const TArray<FVector_NetQuantize>& HitTargets)
{
	...
	if (MuzzleFlashSocket)
	{
		...
		TMap<AWS_Player*, uint32> HitMap;
		TMap<AWS_Player*, uint32> HeadShotHitMap;
		for (FVector_NetQuantize HitTarget : HitTargets)
		{
			FHitResult FireHit;
			WeaponTraceHit(Start, HitTarget, FireHit);

			AWS_Player* BlasterCharacter = Cast<AWS_Player>(FireHit.GetActor());
			if (BlasterCharacter)
			{
		...
		TArray<AWS_Player*> HitCharacters;

		// Maps Character hit to total damage
		TMap<AWS_Player*, float> DamageMap;
		...
		{
			BlasterOwnerCharacter = BlasterOwnerCharacter == nullptr ? Cast<AWS_Player>(OwnerPawn) : BlasterOwnerCharacter;
			BlasterOwnerController = BlasterOwnerController == nullptr ? Cast<AWS_PlayerControll>(InstigatorController) : BlasterOwnerController;
